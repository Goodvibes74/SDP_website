...
The base template provides the site-wide layout (navigation bar, footer, etc.) and uses Bootstrap for responsiveness. For example, we load static assets with \texttt{\{%\ load static\%} and define a content block for child templates. The relevant code is shown below:
\begin{lstlisting}[language=HTML]
{% comment %} Base template with navbar, container, and footer {% endcomment %}
{% load static %}
<!DOCTYPE html>
<html>
<head>
...
<body>
    {% comment %} Navbar with brand and links {% endcomment %}
    ...
    {% comment %} Footer with social icons {% endcomment %}
...
<div class="jumbotron text-center bg-light">
    ...
    <a class="btn btn-primary btn-lg" href="{% url 'browse_sales' %}">
        Browse Sales
    </a>
</div>
{% endblock %}
...
\bibitem{django-static} Django documentation, *How to manage static files*, referring to using `{% load static %}` and `STATIC_URL` (Django 5.2)&#8203;:contentReference[oaicite:6]{index=6}.
